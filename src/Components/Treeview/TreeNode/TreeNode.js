...
const TreeNode = props => {
  ...
  let output = null;
  if (props.thisK.state.treeInfo) {
      const {d, f} = props.thisK.state.treeInfo.treeLables
    output = parseInt(props.lang) === 1 ? props.config[d] : props.config[f];
  } else {
    output = parseInt(props.lang) === 1 ? props.config.form_d_name : props.config.form_f_name;
