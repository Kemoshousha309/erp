...
import { checkInputValidity, reflectOuterState, label } from "../handlers";
...
class AsyncSelectField extends PureComponent {
  ...
  handleClose = (event) => {
    this.props.screen.async_lang_no_options(this.state.dropList);
  ...
  render() {
    ...
    if (field.options) {
      options = field.options.map((op) => {
        return (
          <li onClick={this.handleSelect} key={op.value} value={op.value}>
            {op.template}
          </li>
        );
      });
