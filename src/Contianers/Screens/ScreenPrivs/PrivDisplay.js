...
const PrivDisplay = (thisK) => {
    ...
    if(user) {
        userInfo = (
            <div className={style.userInfo}>
                    ...
                    <Avatar alt="user" src="https://via.placeholder.com" className={style.avatar}/>
                    </div>
    ...
    let privTable = <p className="text-center">{t('prepare_privs', lang_table, lang_no)}....  </p>
                ...
                <div id="form_priv" className={style.table_container}>
                   ...
                   <table style={TableStyle} className="table table-bordered table-dark text-center w-100">
                    <thead>
                        <tr>
                            {
                                priv_arr1.map(i => <th scope="col">{t(i.title, lang_table, lang_no)}</th>)
                            }   
                        </tr>
                    </thead>
                    <tbody>
                    ...
                    <thead>
                        <tr>
                            {
                                priv_arr2.map(i => <th scope="col">{t(i.title, lang_table, lang_no)}</th>)
                            }   
                        </tr>
                    </thead>
                    <tbody>
                ...
                <div id="form_priv" className={style.table_container}>
                   ...
                   <table style={TableStyle} className={classes}>
                    <thead>
                        <tr>
                            {
                                priv_arr.map(i => <th scope="col">{t(i.title, lang_table, lang_no)}</th>)
                            }   
                        </tr>
                    </thead>
                    <tbody>
